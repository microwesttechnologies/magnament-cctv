...
<body class="min-h-dvh bg-background text-foreground antialiased overscroll-y-contain" x-data x-init="$store.shell.init()">
    ...
    <nav class="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface/95 pb-[env(safe-area-inset-bottom)] backdrop-blur overscroll-contain" aria-label="Navegación técnico">
        <div class="mx-auto grid max-w-lg grid-cols-4 gap-2 px-2 py-2">
            @foreach ([
                ...
                <a href="{{ $href }}" class="relative flex min-h-12 flex-col items-center justify-center rounded-md px-2 py-2 text-xs font-medium transition duration-150 motion-reduce:transition-none {{ $active === $key ? 'bg-accent/10 text-accent' : 'text-foreground-muted' }}">
                    ...
                    <span>{{ $label }}</span>
                    @if ($key === 'notifications' && ($unreadNotifications ?? 0) > 0)
                        <span class="absolute right-3 top-1 inline-flex min-h-4 min-w-4 items-center justify-center rounded-full bg-destructive px-1 text-[10px] font-semibold text-white">{{ $unreadNotifications > 9 ? '9+' : $unreadNotifications }}</span>
                    @endif
                </a>
            @endforeach
        </div>
